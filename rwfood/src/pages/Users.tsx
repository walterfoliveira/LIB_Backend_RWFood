...
const Users = () => {
  ...
  const initState: IUser = {
    ...
    idCompany: 1,
    status: 1,
    level: 0, //0=Atendente, 1=Gerente, 2=Admin, 3=SuperAdmin
    name: '',
    mail: '',
    cell: '',
    document: '',
  ...
  const [dataSource, setDataSource] = useState<IUser>(initState);
  ...
  const [empresaList, setEmpresaList] = useState<IUser[]>([])
  const [empresaListFilter, setEmpresaListFilter] = useState<IUser[]>([])
  ...
  const handleInsert = async (confimado: boolean, data: IUser) => {
    setRespModal(!respModal)

    if (confimado) {

      //faz requisicao de Insert (POST)
      var response = await userService.insertUser(data)
    ...
    if (filterInput !== '') {
      const res = empresaList.filter(
        (item) =>
          ...
          item.mail.toLowerCase().includes(filterInput.toLowerCase()) ||
          item.cell.toLowerCase().includes(filterInput.toLowerCase())
      )
  ...
  const handleGetCompanyAll = async () => {
    ...
    var response = await userService.getAllUser(1)
        ...
        <div className="flex flex-col w-full justify-between mb-5">
          <div className="grid grid-cols-4 gap-4">
            {empresaListFilter.map((item) => (
              <CardUser key={item.id} item={item} empresaList={empresaList} setEmpresaList={setEmpresaList} />
            ))}
          </div>
        </div>
      )}
